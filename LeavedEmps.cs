...
namespace HostelMS
{
    public partial class FormLeavedEmps : Form
    {
        public FormLeavedEmps()
        {
            InitializeComponent();
        }
        ...
        private void FormLeavedEmps_Load(object sender, EventArgs e)
        {
            using (SqlConnection sqlcon = new SqlConnection(constring))
            {
                try
                {
                    ...
                    {
                        selcmd.Parameters.AddWithValue("@wksts", "No");
                        SqlDataAdapter sda = new SqlDataAdapter(selcmd);
                        DataTable dt = new DataTable();
                        sda.Fill(dt);

                        DGVLeftEmps.DataSource = dt;
                        LbTRC.Text = DGVLeftEmps.Rows.Count.ToString();
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine(ex.Message, "EmpsLoad");
                }
            }
        }
    }
